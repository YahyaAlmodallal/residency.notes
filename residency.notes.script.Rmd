...
cons.data <- cons.data %>%
  mutate(log_total_time = log(total.time + 1),  # Adding 1 to avoid log(0) if any zeros exist
         smart_phrases_count = str_count(smartphraseids, "\\["))  # Count "[" in smartphraseids
...
# Function to calculate correlation for bootstrapping
correlation_function <- function(data, indices) {
  d <- data[indices,]  # allows bootstrapping to sample with replacement
  cor(d[[1]], d[[2]], use = "complete.obs")  # Calculates correlation
}


# Fitting the model
model <- lm(total.time ~ `%copied` + `%voice` + smart_phrases_count + weekend + pm + 
            notesize + resident.time + staff.time + iteration + starttostop + 
            sources + resident.percent, data = cons.data)
