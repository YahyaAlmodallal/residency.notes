...
```{r, studying variances}

# If car package is not installed, install it first
# install.packages("car")
library(car)

# Assuming `notestatus` is the grouping variable
leveneTest(cons.data$total.time, cons.data$pgy, center = median)


# Bartlett's test with `notestatus` as the grouping factor
bartlett.test(cons.data$total.time, cons.data$pgy)
...
modified_df <- combined_df %>%
  select(variable, group1, N1, Real_Mean1, LCI1, UCI1, p.adj, p.adj.signif) %>%
  distinct()
...
g2 <- ggplot(result_df, aes(x = Label, y = Real_Mean2, group = variable, color = variable)) +
  ...
  geom_text(aes(label = paste("N =", Actual_N), y = 50),  # Adjust y position to minimize overlap
  ...
  ylim(0, max(result_df$CI_Real_Mean2_Upper) + 10)  # Dynamically set y-axis limit to accommodate text

print(g2)

ggsave("figure2.png", plot = g2, width = 10, height = 5)
...
combinations <- list(
  ...
  list(cont_var = "total.time", cat_var = "authorsservice"),
...
modified_df <- combined_df %>%
  select(variable, group2, group1, N, Real_Mean2, CI_Real_Mean2_Lower, CI_Real_Mean2_Upper, p.adj, p.adj.signif) %>%
  distinct()

# Filter rows where 'group2' is in the allowed author services
filtered_df <- modified_df %>%
...
# Update the plotting code to use the new labels with ordered levels
g3 <- ggplot(result_df, aes(x = Label, y = Real_Mean2, group = variable, color = variable)) +
  geom_point(position = position_dodge(width = 0.5)) +  # Position dodge to separate the points
  geom_errorbar(aes(ymin = CI_Real_Mean2_Lower, ymax = CI_Real_Mean2_Upper), 
                position = position_dodge(width = 0.5), width = 0.1) +
  labs(title = "Whisker Plot of Average Note Time by Author's type & Author's Service",
       x = "Author's Service (Count)",
       y = "Average Time (min)",
       color = "Author's type") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for better readability

ggsave("figure3.png", plot = g3, width = 10, height = 6)
...
post_hoc_results <- post_hoc_results %>%
  left_join(descriptive_stats %>%
              rename(group1_notetype = notetype, group1_pgy = pgy) %>%
              select(group1_notetype, group1_pgy, N, Real_Mean1 = Real_Mean), 
            by = c("notetype1_split" = "group1_notetype", "pgy1_split" = "group1_pgy")) %>%
  left_join(descriptive_stats %>%
              rename(group2_notetype = notetype, group2_pgy = pgy) %>%
              select(group2_notetype, group2_pgy, Real_Mean2 = Real_Mean), 
...
modified_df <- post_hoc_results %>%
  select(notetype2_split, pgy2_split, N, Real_Mean2, CI_Real_Mean2_Lower, CI_Real_Mean2_Upper, p.adj, p.adj.signif) %>%
  distinct()
...
# Create a new label that combines note type and count
pgycounts$Label <- paste(pgycounts$pgy, " (", pgycounts$Counts, ")", sep="")

# Sort 'typecounts' by 'Counts' in descending order
pgycounts <- pgycounts[order(-pgycounts$Counts), ]

# Assuming allowed_notetypes is already defined as you mentioned
allowed_notetypes <- c("Progress Notes", "Clinic Note", "Discharge Summary", "H&P", "Flowsheet Note", 
                       "Emergency Department", "Procedures", "Telephone Encounter", "Event", "Consults")

# Filter the dataframe
result_df <- result_df[result_df$notetype2_split %in% allowed_notetypes, ]

# Update the plotting code to use the new labels with ordered levels
g4 <- ggplot(result_df, aes(x = notetype2_split, y = Real_Mean2, group = pgy2_split, color = pgy2_split)) +
  geom_point(position = position_dodge(width = 0.5)) +  # Position dodge to separate the points
  geom_errorbar(aes(ymin = CI_Real_Mean2_Lower, ymax = CI_Real_Mean2_Upper), 
                position = position_dodge(width = 0.5), width = 0.1) +
  labs(title = "Whisker Plot of Average Resident Note Time by Note Type & PGY",
       x = "Note Type",
       y = "Average Time (min)",
       color = NULL) +
  theme_minimal() +
...
print(g4)
ggsave("figure4.png", plot = g4, width = 10, height = 6)
...
```{r}
 ...
 cons.data <- cons.data %>%
  select(notetype, other.time) %>%
  mutate(other.time := ifelse(other.time <= 0, NA, other.time)) %>%
  filter(!is.na(other.time)) %>%
  mutate(log_transformed = log(other.time))

  # Descriptive statistics on log-transformed data
  descriptive_stats <- cons.data %>%
    group_by(notetype) %>%
    summarise(
      Count_Non_NA = n(),
      Real_Mean = exp(mean(log_transformed, na.rm = TRUE)),
...
# Filter data for groups that have at least 2 observations
  ...
  # Welch's ANOVA on log-transformed data
  if (nrow(cons.data) > 1) {
    anova_results <- cons.data %>%
      welch_anova_test(log_transformed ~ notetype)
    } 

  # Games-Howell post-hoc test on log-transformed data
  post_hoc_results <- cons.data %>%
    games_howell_test(log_transformed ~ notetype)
  ...
  combined_results <- combined_results %>%
    left_join(descriptive_stats %>% select(notetype, N1 = N, LCI1 = ci_lower, UCI1 = ci_upper, sd1 = sd, se1 = SE, Real_Mean1 = Real_Mean), by = c("group1" = "notetype")) %>%
    left_join(descriptive_stats %>% select(notetype, N2 = N, LCI2 = ci_lower, UCI2 = ci_upper, sd2 = sd, se2 = SE, Real_Mean2 = Real_Mean), by = c("group2" = "notetype")) %>%
    mutate(
      Mean_Difference = Real_Mean2 - Real_Mean1,
      Estimated_Real_Mean2 = Real_Mean1 * exp(estimate),
      CI_Real_Mean2_Lower = Real_Mean1 * exp(conf.low),
