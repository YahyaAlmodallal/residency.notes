...
columns <- c("total.time", "resident.time", "staff.time", "other.time", "notesize", "%copied", "%voice", "starttostop", "smartphraseids", "resident.percent")
...
```{r, per class and figure 2, echo=FALSE}
...
calc_pairwise_stats <- function(data, variable, min_obs = 2) {
  data <- data %>%
    mutate(log_value = log(.data[[variable]]))
  ...
  summary_stats <- data %>%
    ...
    summarize(
      ...
      ci_lower = mean(log_value, na.rm = TRUE) - qt(0.975, df = n() - 1) * sd(log_value, na.rm = TRUE) / sqrt(n()),
      ci_upper = mean(log_value, na.rm = TRUE) + qt(0.975, df = n() - 1) * sd(log_value, na.rm = TRUE) / sqrt(n()),
      n = n(),
      .groups = 'drop'
    ) %>%
    filter(n >= min_obs)
  
  notetypes <- summary_stats$notetype
  
  pairwise_results <- do.call(rbind, lapply(notetypes, function(type1) {
    do.call(rbind, lapply(notetypes, function(type2) {
      if (type1 != type2) {
        group1 <- data %>% filter(notetype == type1) %>% pull(log_value)
        group2 <- data %>% filter(notetype == type2) %>% pull(log_value)
        
        if (length(group1) >= min_obs && length(group2) >= min_obs) {
          t_test <- tryCatch(t.test(group1, group2), error = function(e) NULL)
          
          if (!is.null(t_test)) {
            mean.group1 <- mean(group1, na.rm = TRUE)
            sd.group1 <- sd(group1, na.rm = TRUE)
            ci_lower.group1 <- mean.group1 - qt(0.975, df = length(group1) - 1) * sd.group1 / sqrt(length(group1))
            ci_upper.group1 <- mean.group1 + qt(0.975, df = length(group1) - 1) * sd.group1 / sqrt(length(group1))
            
            mean.group2 <- mean(group2, na.rm = TRUE)
            sd.group2 <- sd(group2, na.rm = TRUE)
            ci_lower.group2 <- mean.group2 - qt(0.975, df = length(group2) - 1) * sd.group2 / sqrt(length(group2))
            ci_upper.group2 <- mean.group2 + qt(0.975, df = length(group2) - 1) * sd.group2 / sqrt(length(group2))
            
            data.frame(
              group1 = type1,
              group2 = type2,
              mean.group1 = exp(mean.group1),
              sd.group1 = exp(sd.group1),
              ci_lower.group1 = exp(ci_lower.group1),
              ci_upper.group1 = exp(ci_upper.group1),
              mean.group2 = exp(mean.group2),
              sd.group2 = exp(sd.group2),
              ci_lower.group2 = exp(ci_lower.group2),
              ci_upper.group2 = exp(ci_upper.group2),
...
# Perform the pairwise comparison for each variable with minimum observation threshold
pairwise_stats_resident_time <- calc_pairwise_stats(cons.data, "resident.time")
pairwise_stats_total_time <- calc_pairwise_stats(cons.data, "total.time")
pairwise_stats_staff_time <- calc_pairwise_stats(cons.data, "staff.time")

# Combine the results into one dataframe
final_pairwise_results <- bind_rows(
  pairwise_stats_resident_time %>% mutate(variable = "resident.time"),
  pairwise_stats_total_time %>% mutate(variable = "total.time"),
  pairwise_stats_staff_time %>% mutate(variable = "staff.time")
) %>%
  select(variable, group1, group2, mean.group1, sd.group1, ci_lower.group1, ci_upper.group1,
         mean.group2, sd.group2, ci_lower.group2, ci_upper.group2, p.value)
...
perform_anova <- function(data, variable) {
  ...
  # Perform ANOVA
  anova_result <- aov(log_value ~ notetype, data = data)
  anova_summary <- summary(anova_result)
  
  # Perform Tukey's HSD post-hoc test if ANOVA is significant
  if (anova_summary[[1]]$`Pr(>F)`[1] < 0.05) {
    tukey_result <- TukeyHSD(anova_result)
    tukey_df <- as.data.frame(tukey_result$notetype) %>%
...
# Perform ANOVA and post-hoc tests for each variable
anova_resident_time <- perform_anova(cons.data, "resident.time")
anova_total_time <- perform_anova(cons.data, "total.time")
anova_staff_time <- perform_anova(cons.data, "staff.time")

# Combine the results into one dataframe
final_anova_results <- bind_rows(
  anova_resident_time %>% mutate(variable = "resident.time"),
  anova_total_time %>% mutate(variable = "total.time"),
  anova_staff_time %>% mutate(variable = "staff.time")
)

# Join the p-value column from final_anova_results to final_pairwise_results
final_combined_results <- final_pairwise_results %>%
  left_join(final_anova_results %>% select(group1, group2, variable, `p adj`), 
            by = c("group1", "group2", "variable"))
