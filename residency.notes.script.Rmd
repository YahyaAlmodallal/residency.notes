...
percent <- cons.data %>%
  ...
  summarise(
    ...
    weekend = sum(weekend, na.rm = TRUE)/60/60/24/429.680185185185*100,
    after.hours = sum(after.hours, na.rm = TRUE)/60/60/24/657.145706018519*100,
    pm = sum(pm, na.rm = TRUE)/60/60/24/941.186527777778*100,
    .groups = "drop"
  )
