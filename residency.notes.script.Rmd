...
perform_analysis <- function(df, cont_var, cat_var) {
  ...
  post_hoc_results <- post_hoc_results %>%
    left_join(descriptive_stats %>% select(!!sym(cat_var), N, Real_Mean1 = Real_Mean), by = c("group1" = cat_var)) %>%
    left_join(descriptive_stats %>% select(!!sym(cat_var), Real_Mean2 = Real_Mean), by = c("group2" = cat_var)) %>%
    mutate(
...
allowed_notetypes <- as.character(typecounts$NoteType)

# Define the list of combinations to analyze
combinations <- list(
  list(cont_var = "resident.time", cat_var = "notetype"),
  list(cont_var = "total.time", cat_var = "notetype"),
...
g2 <- ggplot(result_df, aes(x = Label, y = Real_Mean2, group = variable, color = variable)) +
  ...
  labs(title = "Whisker Plot of Average Note Time by Author's type & Note Type",
       x = "Note Type (Count)",
       y = "Average Time (min)",
       color = "Author's Type") +
  theme_minimal() +
...
print(g1)
ggsave("figure2.png", plot = g2, width = 10, height = 8)
...
print(g3)
ggsave("figure3.png", plot = g3, width = 10, height = 8)

```


```{r, resident note time by class and year}

per.resident <- readRDS("per.resident.rds")

# Recode specific labels in pgy
per.resident$pgy <- dplyr::recode(per.resident$pgy,
                          "pgy1.21.22" = "PGY-1, FY 21-22",
                          "pgy1.22.23" = "PGY-1, FY 22-23",
                          "pgy1.23.24" = "PGY-1, FY 23-24",
                          "pgy2.21.22" = "PGY-2, FY 21-22",
                          "pgy2.22.23" = "PGY-2, FY 22-23",
                          "pgy2.23.24" = "PGY-2, FY 23-24",
                          "pgy3.21.22" = "PGY-3, FY 21-22",
                          "pgy3.22.23" = "PGY-3, FY 22-23",
                          "pgy3.23.24" = "PGY-3, FY 23-24")

# Create a table of note services
pgy_table <- table(per.resident$pgy)

# Convert the table to a dataframe
pgycounts <- data.frame(
  pgy = names(pgy_table),
  Counts = as.integer(pgy_table)
)

# Sort the dataframe by counts in descending order and keep the top 10
pgycounts <- pgycounts[order(pgycounts$pgy), ]
pgycounts <- head(pgycounts, 10)

# Extract the NoteType column to create a character vector of allowed note types
allowed_pgy <- as.character(pgycounts$pgy)

# Define the list of combinations to analyze
combinations <- list(
  list(cont_var = "resident.time", cat_var = "pgy")
)
...
# Filter data for groups that have at least 2 observations
  ...
  # Merge to add Real Means
  post_hoc_results <- post_hoc_results %>%
    left_join(descriptive_stats %>% select(notetype, N, Real_Mean1 = Real_Mean), by = c("group1" = "notetype")) %>%
    left_join(descriptive_stats %>% select(notetype, Real_Mean2 = Real_Mean), by = c("group2" = "notetype")) %>%
    mutate(
      Mean_Difference = Real_Mean2 - Real_Mean1,
      Estimated_Real_Mean2 = Real_Mean1 * exp(estimate),
      CI_Real_Mean2_Lower = Real_Mean1 * exp(conf.low),
...
# Merge this label with the result_df dataframe based on NoteType
result_df <- merge(result_df, pgycounts, by.x="group2", by.y="pgy")

# Set the factor levels for 'Label' in 'result_df' based on the order in 'typecounts'
result_df$Label <- factor(result_df$Label, levels = pgycounts$Label)

# Update the plotting code to use the new labels with ordered levels
g4 <- ggplot(result_df, aes(x = Label, y = Real_Mean2, group = variable, color = variable)) +
  geom_point(position = position_dodge(width = 0.5)) +  # Position dodge to separate the points
  geom_errorbar(aes(ymin = CI_Real_Mean2_Lower, ymax = CI_Real_Mean2_Upper), 
                position = position_dodge(width = 0.5), width = 0.1) +
  labs(title = "Whisker Plot of Average Note Time by Author's type & Author's Service",
...
print(g4)
ggsave("figure4.png", plot = g4, width = 10, height = 8)
...
post_hoc_results <- post_hoc_results %>%
  separate(group2, into = c("notetype_split", "pgy_split"), sep = "\\.", extra = "merge")
