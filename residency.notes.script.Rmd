...
# 1: total time vs resident time vs attending time (changes in sum in days over blocks)
total_staff <- unlist(lapply(split_data_list, function(df) sum(df$staff.time, na.rm = TRUE) / (60*60)))
total_res <- unlist(lapply(split_data_list, function(df) sum(df$resident.time, na.rm = TRUE) / (60*60))) 
total_all <- unlist(lapply(split_data_list, function(df) sum(df$total.time, na.rm = TRUE) / (60*60)))

y1 <- data.frame(block_start_dates, time = total_all, author= "Total")
y2 <- data.frame(block_start_dates, time = total_res, author= "Resident")
y3 <- data.frame(block_start_dates, time = total_staff, author= "Staff")
all_authors <- rbind(y1, y2, y3)

plot_author <- ggplot(all_authors, aes(x = block_start_dates, y = time, color = author)) +
  geom_line() +
  labs(title = "Monthly Trend of Total Time by Author",
       x = "Block",
       y = "Total Time (hours)",
       color = "Note Type") +
  theme_minimal() + 
  scale_x_date(breaks = as.Date(c("2021-07-01", "2021-10-01", "2022-01-01", "2022-04-01", "2022-07-01", "2022-10-01", "2023-01-01", "2023-04-01", "2023-07-01", "2023-10-01", "2024-01-01", "2024-04-01")), 
               labels = c("Jul 2021", "Oct 2021", "Jan 2022", "Apr 2022", "Jul 2022", "Oct 2022", "Jan 2023", "Apr 2023", "Jul 2023", "Oct 2023", "Jan 2024", "Apr 2024")) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5, hjust = 1),
        axis.title.x = element_text(margin = margin(t = 20)))
print(plot_author)

ggsave("plot_author.png", plot = plot_author, width = 10, height = 6, dpi = 600, device = "png")


# 2. PGY 1 vs 2 vs 3 (changes in sum in days over blocks)
filtered_pgy1 <- lapply(split_data_list, function(df) {
  pgy_block <- df %>%
    filter(pgy %in% c("pgy1.21.22", "pgy1.22.23", "pgy1.23.24"))
}) 
total_pgy1 <- do.call(rbind, lapply(filtered_pgy1, calculate_statistics))

filtered_pgy2 <- lapply(split_data_list, function(df) {
  pgy_block <- df %>%
    filter(pgy %in% c("pgy2.21.22", "pgy2.22.23", "pgy2.23.24"))
})
total_pgy2 <- do.call(rbind, lapply(filtered_pgy2, calculate_statistics))

filtered_pgy3 <- lapply(split_data_list, function(df) {
  pgy_block <- df %>%
    filter(pgy %in% c("pgy3.21.22", "pgy3.22.23", "pgy3.23.24"))
})
total_pgy3 <- do.call(rbind, lapply(filtered_pgy3, calculate_statistics))

y1 <- data.frame(block_start_dates, time=total_pgy1$TotalTimeDays, type="PGY-1")
y2 <- data.frame(block_start_dates, time=total_pgy2$TotalTimeDays, type="PGY-2")
y3 <- data.frame(block_start_dates, time=total_pgy3$TotalTimeDays, type="PGY-3")
all_pgys <- rbind(y1, y2, y3)

plot_pgys <- ggplot(all_pgys, aes(x = block_start_dates, y = time, color = type)) +
  geom_line() +
  labs(title = "Monthly Trend of Total Note Time by PGY",
       x = "Block",
       y = "Total Time (days)",
       color = "Year") +
  theme_minimal() + 
...
ggsave("plot_pgys.png", plot = plot_pgys, width = 10, height = 6, dpi = 600, device = "png")

# 3. HP vs Clinic note vs DS vs consult notes
y1 <- data.frame(block_start_dates, time=block_data$HP, type="H&P")
y2 <- data.frame(block_start_dates, time=block_data$Clinic, type="Clinic")
y3 <- data.frame(block_start_dates, time=block_data$Discharge, type="Discharge Summary")
y4 <- data.frame(block_start_dates, time=block_data$Consult, type="Consult")
all_types <- rbind(y1, y2, y3, y4)

plot_type <- ggplot(all_types, aes(x = block_start_dates, y = time, color = type)) +
  geom_line() +
  labs(title = "Monthly Trend of Average Time by Note Type",
       x = "Block",
...
ggsave("plot_type.png", plot = plot_type, width = 10, height = 6, dpi = 600, device = "png")


# 4. Ambulatory vs ED vs Inpatient 
y1 <- data.frame(block_start_dates, number=block_data$Ambulatory, service="Ambulatory")
y2 <- data.frame(block_start_dates, number=block_data$EDNotes, service="ED")
y3 <- data.frame(block_start_dates, number=block_data$Inpatient, service="Inpatient")
all_services <- rbind(y1, y2, y3)

plot_services <- ggplot(all_services, aes(x = block_start_dates, y = number, color = service)) +
  geom_line() +
  labs(title = "Monthly Trend of Total Notes by Service",
       x = "Block",
       y = "Total Notes",
       color = "Service") +
  theme_minimal() + 
...
ggsave("plot_services.png", plot = plot_services, width = 10, height = 6, dpi = 600, device = "png")


# 5. pm vs after.hours
y1 <- data.frame(block_start_dates, time=block_data$PMTimeDays, service="PM")
y2 <- data.frame(block_start_dates, time=block_data$AfterHoursTimeDays, service="After Hours")
y3 <- data.frame(block_start_dates, time=block_data$TotalTimeDays, service="Total Time")
all_times <- rbind(y1, y2, y3)

plot_times <- ggplot(all_times, aes(x = block_start_dates, y = time, color = service)) +
  geom_line() +
  labs(title = "Monthly Trend Outside of Typical Hours",
       x = "Block",
       y = "Total Time (days)",
       color = "Time Category") +
  theme_minimal() + 
...
ggsave("plot_times.png", plot = plot_times, width = 10, height = 6, dpi = 600, device = "png")
...
percent.days.per.type <- cons.data %>%
  group_by(notetype) %>%
  summarise(
    type.time = sum(resident.time)/60/60/24/2842.8*100,
    .groups = "drop"
  )
...
pgy1 <- per.resident %>%
  ...
  group_by(noteid) %>%
  summarise(
    resident.time = sum(resident.time),
    weekend = sum(weekend, na.rm = TRUE),
    after.hours = sum(after.hours, na.rm = TRUE),
    pm = sum(pm, na.rm = TRUE),
    across(everything(), first),
...
# Create a combined dataframe with log-transformed 'resident.time'
data_combined <- bind_rows(
  mutate(pgy1, year = "PGY1", log_time = log(resident.time)),
  mutate(pgy2, year = "PGY2", log_time = log(resident.time)),
  mutate(pgy3, year = "PGY3", log_time = log(resident.time))
) %>%
  select(year, log_time)

# Perform Welch's ANOVA
anova_results <- data_combined %>%
  welch_anova_test(log_time ~ year)

# Conduct Games-Howell post-hoc test
posthoc_results <- data_combined %>%
  games_howell_test(log_time ~ year)

# Calculate additional statistics
additional_stats <- data_combined %>%
  group_by(year) %>%
  summarise(
    Count_Non_NA = n(),
    Real_Mean = exp(mean(log_time, na.rm = TRUE))/60,
    sd = exp(sd(log_time, na.rm = TRUE))/60,
    N = sum(!is.na(log_time)),
    sd_log = sd(log_time, na.rm = TRUE),
    SE = exp(sd_log) / sqrt(N),
    ci_lower = exp(mean(log_time, na.rm = TRUE) - qt(0.975, df = N - 1) * sd_log / sqrt(N))/60,
    ci_upper = exp(mean(log_time, na.rm = TRUE) + qt(0.975, df = N - 1) * sd_log / sqrt(N))/60,
    .groups = 'drop'
  )

# Merge the posthoc results with the additional statistics
final_results <- left_join(posthoc_results, additional_stats, by = "year")
...
print(
    ...
    paste("PGY-1, PGY-2, PGY-3: percent of all notes is", nrow(pgy1)/156898*100, nrow(pgy2)/156898*100, nrow(pgy3)/156898*100),
    
    paste("PGY-1, PGY-2, PGY-3: percent of resident days", sum(pgy1$resident.time)/60/60/24/2842.78020833333*100, sum(pgy2$resident.time)/60/60/24/2842.78020833333*100, sum(pgy3$resident.time)/60/60/24/2842.78020833333*100),
    
    paste("PGY-1, PGY-2, PGY-3: percent of resident pm days", sum(pgy1$pm)/60/60/24/941.186527777778*100, sum(pgy2$pm)/60/60/24/941.186527777778*100, sum(pgy3$pm)/60/60/24/941.186527777778*100),
    
    paste("PGY-1, PGY-2, PGY-3: percent of resident after hours days", sum(pgy1$after.hours)/60/60/24/657.145706018519*100, sum(pgy2$after.hours)/60/60/24/657.145706018519*100, sum(pgy3$after.hours)/60/60/24/657.145706018519*100),
    
    paste("PGY-1, PGY-2, PGY-3: percent of resident weekend days", sum(pgy1$weekend)/60/60/24/429.680185185185*100, sum(pgy2$weekend)/60/60/24/429.680185185185*100, sum(pgy3$weekend)/60/60/24/429.680185185185*100),
    
    paste("mean and SD time in hours from start to stop are:", exp(mean(log(cons.data$starttostop), na.rm = TRUE))), exp(sd(log(cons.data$starttostop), na.rm = TRUE)),
    
    paste("mean and SD number of charachters in a note:", exp(mean(log(cons.data$notesize), na.rm = TRUE))), exp(sd(log(cons.data$notesize), na.rm = TRUE)),

      )
)

# Replace zeros with NA in %copied and %voice
cons.data <- cons.data %>%
  mutate(
    `%copied` = na_if(`%copied`, 0),
    `%voice` = na_if(`%voice`, 0)
  )

# Define the custom function to calculate statistics
calculate_stats <- function(data, column) {
  data <- data %>%
    mutate(!!sym(column) := na_if(!!sym(column), 0))
  
  stats <- data %>%
    summarise(
      count = sum(!is.na(!!sym(column))),
      range = diff(range(!!sym(column), na.rm = TRUE)),
      min = min(!!sym(column), na.rm = TRUE),
      max = max(!!sym(column), na.rm = TRUE),
      exp_log_mean = exp(mean(log(!!sym(column)), na.rm = TRUE)),
      exp_log_sd = exp(sd(log(!!sym(column)), na.rm = TRUE))
    )
  ...
  message <- paste(
    ...
    "\nMin:", stats$min,
    "\nMax:", stats$max,
    "\nExp(log(mean)):", stats$exp_log_mean,
    "\nExp(log(SD)):", stats$exp_log_sd
  )
  
  return(message)
}

# Apply the function to %copied, %voice, and iteration columns
message_copied <- calculate_stats(cons.data, "%copied")
message_voice <- calculate_stats(cons.data, "%voice")
message_iteration <- calculate_stats(cons.data, "iteration")
message_sources <- calculate_stats(cons.data, "sources")
