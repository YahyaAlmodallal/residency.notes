...
perform_anova <- function(data, variable, alpha = 0.01) {
  data <- data %>%
    mutate(log_value = log(.data[[variable]]))
  ...
  if (anova_summary[[1]]$`Pr(>F)`[1] < 0.05) {
    ...
    # Transform the results back from the log scale
    tukey_df <- tukey_df %>%
      mutate(
        ratio = exp(diff),
        ci_lower = exp(lwr),
...
final_anova_results <- final_anova_results %>% filter(significant == "Yes")
