...
calculate_statistics <- function(df) {
  ...
  total_time_days <- sum(df$total.time) / (60 * 60 * 24)
  mean_time_mins <- exp(mean(log(df$total.time / 60), na.rm = TRUE))
  sd_time_mins <- exp(sd(log(df$total.time / 60), na.rm = TRUE))
  median_time_mins <- median(df$total.time / 60)
  max_time_mins <- max(df$total.time / 60)
  quantile_25th_time_mins <- quantile(df$total.time / 60, probs = 0.25)
  quantile_75th_time_mins <- quantile(df$total.time / 60, probs = 0.75)
  weekend_time_days <- sum(df$weekend) / (60 * 60 * 24)
  percentage_weekend <- sum(df$weekend) / sum(df$total.time) * 100
  pm_time_days <- sum(df$pm) / (60 * 60 * 24)
  percentage_pm <- sum(df$pm) / sum(df$total.time) * 100
  after_hours_time_days <- sum(df$after.hours) / (60 * 60 * 24)
  percentage_after_hours <- sum(df$after.hours) / sum(df$total.time) * 100
  ...
  mean_hp <- ifelse(any(df$notetype == "H&P"), exp(mean(log(df$total.time[df$notetype == "H&P"] / 60), na.rm = TRUE)), NA)
  mean_CN <- ifelse(any(df$notetype == "Clinic Note"), exp(mean(log(df$total.time[df$notetype == "Clinic Note"] / 60), na.rm = TRUE)), NA)
  mean_DS <- ifelse(any(df$notetype == "Discharge Summary"), exp(mean(log(df$total.time[df$notetype == "Discharge Summary"] / 60), na.rm = TRUE)), NA)
  mean_Cs <- ifelse(any(df$notetype == "Consults"), exp(mean(log(df$total.time[df$notetype == "Consults"] / 60), na.rm = TRUE)), NA)
  mean_PN <- ifelse(any(df$notetype == "Progress Notes"), exp(mean(log(df$total.time[df$notetype == "Progress Notes"] / 60), na.rm = TRUE)), NA)
  mean_TE <- ifelse(any(df$notetype == "Telephone Encounter"), exp(mean(log(df$total.time[df$notetype == "Telephone Encounter"] / 60), na.rm = TRUE)), NA)
  mean_ED <- ifelse(any(df$notetype == "Emergency Department"), exp(mean(log(df$total.time[df$notetype == "Emergency Department"] / 60), na.rm = TRUE)), NA)
  mean_P <- ifelse(any(df$notetype == "Procedures"), exp(mean(log(df$total.time[df$notetype == "Procedures"] / 60), na.rm = TRUE)), NA)
  ...
  data.frame(
    AllNotes = all_notes,
    InpatientNotes = inpatient_notes,
    AmbulatoryNotes = ambulatory_notes,
    EDNotes = ed_notes,
    TelephoneNotes = telephone_notes,
    TotalTimeDays = total_time_days,
    MeanTimeMins = mean_time_mins,
    ...
    PercentageWeekend = percentage_weekend,
    PMTimeDays = pm_time_days,
    PercentagePM = percentage_pm,
    AfterHoursTimeDays = after_hours_time_days,
    PercentageAfterHours = percentage_after_hours,
    MeanNoteSize = mean_note_size,
    SDNoteSize = sd_note_size,
    CountDictated = count_dictated,
    MeanDictation = mean_dictation,
    SDDictation = sd_dictation,
    CountCopied = count_copied,
    MeanCopied = mean_copied,
    SDCopied = sd_copied,
    SmartphraseMean = smartphrase_mean,
    TimeToCompleteMean = time_to_complete_mean,
    ...
    HPMean = mean_hp,
    ClinicNoteMean = mean_CN,
    DischargeSummaryMean = mean_DS, 
    ConsultsMean = mean_Cs, 
    ProgressNotesMean = mean_PN, 
    TelephoneEncounterMean = mean_TE, 
    EmergencyDeptMean = mean_ED, 
    ProceduresMean = mean_P
  )
}
...
library(GGally)
ggpairs(per.pgy1, columns = c("AllNotes", "MeanTimeMins", "PercentageWeekend", "PercentagePM"))
